...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6 flex-wrap gap-4">
        <h1 class="text-2xl text-blue-400">Opening Explorer: {{ $openingName ?? 'Unknown Opening' }}</h1>
                ...
                <div class="flex-1 p-4 overflow-y-auto flex flex-col gap-3">
                     @foreach($comments as $ply => $comment) 
                        <div class="bg-gray-700 p-3 rounded text-sm">
                            <div class="text-gray-400 mb-1">Move {{ $ply }}:</div>
                    ...
                    <div class="flex flex-col gap-1 text-sm">
                        ...
                        @for($i = 0; $i < count($moves); $i += 2)
                            <div class="flex gap-1">
                                ...
                                <div class="flex-1 px-3 py-1 rounded hover:bg-gray-700 cursor-pointer" data-ply="{{ $i+1 }}">
                                    {{ $whiteMove ? getPieceIcon($whiteMove->piece_number) . ' ' . $whiteMove->from_position . ' → ' . $whiteMove->to_position : '' }}
                                </div>

                                <div class="flex-1 px-3 py-1 rounded hover:bg-gray-700 cursor-pointer" data-ply="{{ $i + 2 }}">
